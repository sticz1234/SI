...
class FuzzyInputVariable_Sigmoid:
    fw = 0.0 #type: float
    c = 0.0 #type: float
    name = None #type: str
    labels = None #type: List[str]
    ...
    def showRegions(self, ax, orientation):
        ...
        if orientation==0:
            ax.add_patch(Eclipse((begin,self.c-self.fw/2),100,self.fw, fill=False, hatch='/',color='r'))
            return [self.c]
        else:
            ax.add_patch(Eclipse((self.c-self.fw/2,begin),self.fw,100, fill=False, hatch='/',color='b'))
            return [self.c]
